...
import type { DailyStocksTableRow } from '../lib/server/db/schema';
...
async function insertDailyStocks({
  ...
  for (let i = 0; i < rowsWithCalculations.length; i += CHUNK_SIZE) {
    ...
    console.log(`Inserted ${i + chunk.length} / ${rowsWithCalculations.length} stocks`);
  }
}
...
async function main() {
  ...
  if (!targetDateArg || Number.isNaN(targetDate.getTime())) {
    console.error('Usage: bun run src/jobs/fetchDailyStocks.ts <YYYY-MM-DD>');
    process.exit(1);
  }
  ...
  console.log(`Saved ${data.results.length} daily rows for ${toDateString(targetDate)}`);
}
